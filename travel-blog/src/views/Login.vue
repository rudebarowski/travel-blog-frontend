  ...
  <div class="min-h-screen bg-gray-100 flex items-center justify-center py-12 px-4 sm:px-6 lg:px-8">
        <div class="max-w-xl w-full space-y-8 bg-white shadow rounded p-10">
          ...
          <form class="mt-8 space-y-6">
            ...
            <div>
              <button :disabled="diasbleButton" @click.prevent="login" type="submit" class="group relative w-full flex justify-center py-3 px-4 border border-transparent text-sm font-medium rounded-md text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
...
export default {
  ...
  computed: {
    diasbleButton() {
      if (
        this.$v.login_data.password.$error ||
        this.$v.login_data.password.$error ||
        this.is_submitting
      ) {
        return true;
